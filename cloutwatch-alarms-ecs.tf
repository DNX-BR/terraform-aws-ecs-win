resource "aws_cloudwatch_metric_alarm" "ecs_high_memory" {
  ...
  alarm_name                = "${try(data.aws_iam_account_alias.current.account_alias, var.alarm_prefix)}-ecs-${var.name}-high-memory"
...
resource "aws_cloudwatch_metric_alarm" "ecs_high_cpu" {
  ...
  alarm_name                = "${try(data.aws_iam_account_alias.current.account_alias, var.alarm_prefix)}-ecs-${var.name}-high-cpu"
   comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "3"
