resource "aws_cloudwatch_metric_alarm" "alb_500_errors" {
  ...
  alarm_name                = "${try(data.aws_iam_account_alias.current.account_alias, var.alarm_prefix)}-ecs-${var.name}-alb-500-errors"
...
resource "aws_cloudwatch_metric_alarm" "alb_400_errors" {
  ...
  alarm_name                = "${try(data.aws_iam_account_alias.current.account_alias, var.alarm_prefix)}-ecs-${var.name}-alb-400-errors"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  evaluation_periods        = "2"
...
resource "aws_cloudwatch_metric_alarm" "alb_latency" {
  ...
  alarm_name                = "${try(data.aws_iam_account_alias.current.account_alias, var.alarm_prefix)}-ecs-${var.name}-alb-latency"
